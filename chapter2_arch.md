...
以下时间估计基于 Raspberry Pi 2/3 硬件，实际时间会因硬件配置和系统负载而异。

**第一阶段：硬件启动 (通常 0-100ms)**
